...
[[kibi-advanced-settings-relationalPanel]]`kibi:relationalPanel` :: Display the Relational Panel in the dashboard tab.
`kibi:relations` :: Relations between index patterns and dashboards.
`kibi:joinLimit` :: Maximum number of source documents in a relation returned to filter the target documents
...
`kibi:graphUseFiltersFromDashboards`  :: Set to true to use filters from dashboards on expansion.
