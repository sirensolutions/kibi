...
where you can set the number of unique values returned from the related index pattern for that relation. 
...
- Potentially, this could return many more (or many less) than the limit set as
  ...
  returned from `B`
- The final number of values returned is always limited by the Elasticsearch `size` parameter
