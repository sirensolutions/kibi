...
are collections of permissions on saved objects and ui contexts; the main purpose of this
system is to hide applications, kibi sections or dashboards on unauthorized indices to end users and avoid
unauthorized changes to configuration objects.

There are two kinds of rules:

 * rules - to set permission for saved objects
 * ui rules - to set permission to view different UI elements,
 applications (e.g.: **timelion**, **access_control**) and
 sections of Kibi (e.g.: **discover**, **management**)
...
Denying access to certain saved object like **dashboard** using the first sets
of rules is usually transparent to the user which means that he will simply
do not see the objects anywhere in Kibi.

Denying access to an application like **timelion**
or Kibi section like **management** will hide the navigation menu elements
and block access on the route level and display an error.
...
Below and error user will see when trying to access **/app/timelion** URL
...
Below and error user will see when trying to access **/app/kibana#/management** URL
