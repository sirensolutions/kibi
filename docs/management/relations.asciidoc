...
[[kibi-join-type]]
==== Join Type
There are three types of joins algorithms available in Kibi/Vanguard.
By default (if not set) the backend will try to choose the best algorithm for particular join 
User can force it using one of the three available options
...
The details description of each algorithm can be found in Vanguard plugin documentation  
