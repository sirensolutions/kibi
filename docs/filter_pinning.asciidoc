...
When you create a filter anywhere in Kibi, the filter conditions display in an oval under the search text
entry box:
...
pattern in Kibi is to use _Dashboard Groups_ to group together dashboard which are based on the same underlying index.
