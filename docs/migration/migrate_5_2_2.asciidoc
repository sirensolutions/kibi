...
This section provides a summary of the upgrade procedure described at
{elastic-ref}/setup-upgrade.html[Upgrading Elasticsearch] with {kibi-name} and
Search Guard specific notes.
...
It is recommended to have a snapshot of the indices in the cluster; if running
a production cluster, you will need to request an updated license for Siren
Federate.
...
configuration to adjust Investigate specific permissions as described in the next
section.

[float]
=== Search Guard configuration changes

This section describes all the {kibi-name} specific changes required to the
Search Guard configuration files; it is advised to backup the files in the
retrieved configuration directory before making changes.
...
If present, replace the `KIBI_COMPOSITE` role with the following definition for
...
Add the `SIREN_CLUSTER`, `SIREN_READONLY` and `SIREN_READWRITE` roles with the
...
If present, replace `KIBI_CLUSTER`, `KIBI_READONLY` and `KIBI_READWRITE` with
the following definitions for backwards compatibility.
...
correct names for the main {kibi-name} (`.siren` by default) and access control
(`.sirenaccess` by default) indices if using a custom configuration**.
...
Siren Alert (previously known as Sentinl) is automatically enabled in
{kibi-name}; if you do not need alerting, it is possible to disable the plugin
by putting the following in `investigate.yml`:
...
If you want to use the alerting features, check that a user named `sentinl` is
defined as described in the example below; **make sure to set the correct names
for watcher and watcher_alarms indices if using a custom configuration**.

[source,yaml]
----
# Permissions for a Siren Alert / Sentinl user.
sentinl:
  cluster:
    - "indices:data/write/bulk*"
...
[float]
===== {kibi-name} user roles

In each existing user role with access to {kibi-name}:
 
- replace `KIBI_MSEARCH` with `SIREN_COMPOSITE`.
- replace `KIBI_COMPOSITE` with `SIREN_COMPOSITE`.
- remove the following permission on the main {kibi-name} index (which was
  named `.kibi` in previous versions) from each user role that has it:
...
If you are using the {kibi-name} access control plugin and want to enable
access control rules on saved objects, ensure that {kibi-name} users **do not**
have access to the {kibi-name} index (which was set to `.kibi` in previous
releases), otherwise they might be able to peek at saved objects by issuing
custom Elasticsearch queries.

Access to the {kibi-name} index was usually granted by the following lines in
previous releases; these must be removed to use access control rules
effectively as the configuration index will be managed exclusively by the
`kibiserver` role:
...
search the {kibi-name} index as well; if you need to hide it, you should
replace the wildcard with an explicit list of indices.

*License management*

To grant a role the permission to upload an {kibi-name} license from the UI,
ensure that the user has the `cluster:admin/plugin/siren/license/*`
permission, e.g.:
...
For information on how to setup Search Guard for JDBC datasources support
see the <<jdbc_datasources>> section after the upgrade.
...
Before upgrading {kibi-name}, make sure to perform the following steps:
 
- backup the configuration index (`.kibi` by default).
- backup the configuration file (`config/kibi.yml`)

Then perform the following steps:

- extract {kibi-name} to a new directory.
...
- execute `bin/investigate upgrade-config`.
- execute `bin/investigate upgrade`.
...
If SSL support was enabled in the previous installation, you will need to
update `investigate.yml` as follows:
...
If you want to enable access control rules on saved objects, you will need to:

- add `acl.enabled: true` to the `investigate_access_control` section;
- specify the Search Guard role that will have administrative access to the ACL
  configuration in `admin_role` (`.sirenadmin` by default);
- specify the name of the index where access control rules will be stored
  (`.sirenaccess` by default).
...
Once ACLs are enabled, you will need to login with a user having the role
specified in `admin_role` and visit Access Control -> ACL to create {kibi-name}
roles with the desired permissions.

For more informations about access control rules on saved objects, please check
the <<searchguard_integration>> chapter.
...
- `kibi_wordcloud`: {kibi-name} now supports the Tag Cloud visualization
  introduced in Kibana 5. Existing Word Cloud visualizations will be migrated
  to Tag Cloud visualizations by the upgrade procedure.
- `heatmap`: the heatmap plugin for Kibana 4.x has been integrated into Kibana
  5 and thus is available in {kibi-name}.  Existing Heatmap visualizations will
  be upgraded automatically, although the color palette will be set to `Greens`
  as most of the palettes in the original plugin were not ported to Kibana 5.
...
the index patterns containing the field by going to Management/Indexes And Relations
in {kibi-name}, opening each pattern and clicking on the refresh button; once
the index pattern is refreshed, the field should appear as aggregatable.
...
[float]
==== From {kibi-name}
To upload the license from {kibi-name}, navigate to the management section.

image::images/management_no-license.png[]
...
{elastic-ref}/breaking_50_search_changes.html#_deprecated_queries_removed[breaking changes]
