...
[float]
==== Join Limit
The number of documents returned from the `source` of the relation is limited by the `kibi:joinLimit` Advanced Setting in the management section.
These `source` documents are then used to filter the documents on the `destination`. In general, the `destination` is the current dashboard.
