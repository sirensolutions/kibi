...
datasources, we also recommend to user credentials with no write privileges as
they are not required by the system.

[float]
[[access-control]]
=== Access control
Kibi is compatible with Elastic Shield and Search Guard to enable index
level access control; the Kibi Enterprise Edition provides extended support
for both products through configurable error handling.
