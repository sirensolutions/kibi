...
The file defines the list of users assigned to each role using the following
form:
...
If the command sgadmin command is executed successfully, a list of the actions
executed and their outcome will be printed on screen, e.g.:
...
`elasticsearch.ssl.verificationMode` to `certificate`, e.g.:
...
If you want to use the Siren Alert plugin, you'll need to specify the Siren
Alert user credentials in the `investigate_access_control.sirenalert` section,
e.g.:
...
image::images/access_control/acl_app.png["The Access control app",align="center"]
