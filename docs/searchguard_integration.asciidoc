...
== X-Pack integration and Kibi access control

If you have an existing Elasticsearch instance with X-Pack installed and wish to integrate with Kibi,
there are a few short steps to take:

First, set the backend parameter of the `kibi_access_control` section of the `kibi.yml` to `xpack`:
...
Second, replace the kibiserver role with the following; make sure to set the correct names for the main kibi (.kibi by default) and access control (.kibiaccess by default) indices if using a custom configuration
...
Once you log in to system you can configure roles from Kibi Access Control UI. You can find example role in below.
