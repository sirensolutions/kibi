...
class DashboardState {
  ...
  syncTimefilterWithDashboard(timeFilter, quickTimeRanges) {
    ...
    // kibi: restore the mode too
    timeFilter.time.mode = this.savedDashboard.timeMode;
  ...
  saveDashboard(toJson, timeFilter) {
    ...
    this.savedDashboard.timeMode = this.savedDashboard.timeRestore ? timeFilter.time.mode : undefined;
    ...
    return this.savedDashboard.save()
      .then((id) => {
        delete this.savedDashboard.locked; // kibi: our lock for the dashboard!
        this.lastSavedDashboardFilters = this.getFilterState();
        this.stateDefaults = getStateDefaults(this.savedDashboard);
