...
This list of plugins is not guaranteed to work on your version of {kibi-name}. Instead, these are plugins that were known to work at some point with {kibi-name} *5.x*. The {kibi-name} installer will reject any plugins that haven't been published for your specific version of {kibi-name}.
...
* https://github.com/ppadovani/KibanaNestedSupportPlugin[Nested Field Support] (ppadovani) - Adds nested field/document support to Kibana.
