...
* TODO: !!! fill this in when REST reimplemented
...
NOTE: Sensitive datasource configuration elements like passwords are encrypted.
...
NOTE: Currently, a query is executed synchronously on the server. Kibi needs then to wait until results are retrieved.
Therefore, long standing queries may block Kibi for an undeterminate amount of time.
