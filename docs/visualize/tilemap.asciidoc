...
NOTE: By default, Kibana uses the http://www.openstreetmap.org/[Open Street Maps service]
...
Tile maps uses the {elastic-ref}search-aggregations-bucket-geohashgrid-aggregation.html[_geohash_] aggregation. Select a field, typically coordinates, from the
drop-down. 
