...
- **enabled**: set to **true** to enable the Kibi access control plugin.
  Defaults to **false**.
- **backend**: backend type of authentication.
...
You may also need to set the `encryptionKey` of X-pack under `kibi_access_control.backends` section.
...
If you are not accessing Elasticsearch over HTTPS, you need to set `kibi_access_control.cookie.secure` to *false*.