...
<<kibi_multi_chart,Multi chart>>:: Display multiple visualization configurations.
...
<<heatmap-chart,Heat map>>:: Display data as a heat map.
...
<<kibi_scatter_plot, Scatter plot>>:: Display data in a scatter plot chart.
...
<<kibi_box_plot, Box plot>>:: Display data in a box plot.
<<bubble_diagram, Kibi Bubble Diagram>>:: Display series of data grouped into packed circles.
<<kibi_horizontal_bar_chart, Horizontal Bar Chart>>:: Graph values in a horizontal bar chart.
