...
where you can set the number of documents returned from the related index pattern for that relation. 
...
The limit is a total upper limit on the number of those documents on the other index pattern for which the `join` field exists.
This limit is dynamically allocated over the shards where the index is located. 

The index pattern in question is then filtered by the documents returned. 
