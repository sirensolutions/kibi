...
[float]
=== Configuration
To configure the visualization select a compatible datasource (typically the provided _Gremlin Server_ datasource)
 ...
 ** *Font awesome* - the icon will be selected using the vector icons from https://fontawesome.github.io/Font-Awesome/[Font Awesome]
 ** *Parameterized Relative Path* - the icon will be selected from the configured relative path. The relative path points to `<investigate_root>/installedPlugins/graph_browser_vis/public/icons/`. You can create subfolders to better organize your custom icons. +
 Examples:
