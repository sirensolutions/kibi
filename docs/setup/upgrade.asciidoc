...
{kibi-name} ships with a command line interface for creating dumps of the `.siren` index and, in case the `ACL` is enabled, the `.kibiaccess` index as well.
...
For example, the following line will dump in `<MY_FOLDER>` the `.siren` index and the `.kibiaccess` index if the option `kibi_access_control.acl.enabled` is true in `siren.yml`:
