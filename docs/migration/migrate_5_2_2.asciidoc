...
Guard configuration files; it is advised to make a copy of the configuration
directory retrieved before the upgrade and apply the changes there, leaving the
backup intact.
...
Sentinl is automatically enabled in Kibi 5; if you do not need alerting is
it possible to disable the plugin by putting the following in `kibi.yml`:
...
saved objects (which allow to restrict access to saved objects), ensure that
Kibi users **do not** have access to the Kibi index (`.kibi` by default),
otherwise they might be able to peek at saved objects by issuing custom
...
see the the Kibi index as well; if you need to hide it, it is recommended to
replace the wildcard with a list of rules for each index.
