...
Kibana visualizations are based on the {elastic-ref}search-aggregations.html[aggregation] feature introduced in Elasticsearch 1.x.
...
[horizontal]
<<enhanced-search-results,Enhanced search results>>:: Display an interactive
table of documents returned by an Elasticsearch query.
<<templated-query-viewer,Templated Query Viewer>>:: Display the results of
queries on external datasources in Jade/Handlebards templates.
<<relational-filter,Relational Filter>>:: Apply cross-dashboard filters
...
<<radar-chart, Radar Chart>>:: Display multivariate data with a two-dimensional chart.
<<graph-browser, Kibi Graph Browser*>>:: Displays a graph visualization starting from a selected document
...
_External Query Terms Filter_ aggregation, which is described in the <<external-datasources>> chapter.
...
include::enhanced-search-results.asciidoc[]

include::templated-query-viewer.asciidoc[]
...
include::radar-chart.asciidoc[]

include::graph-browser.asciidoc[]
