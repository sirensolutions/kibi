[[discover]]
== Discover
You can interactively explore the data that is in the underlying Elasticsearch store from the Discover page. You have access to every document in every index that
...
<<advanced-settings,Advanced Settings>>. By default, the table shows the localized version of the time field specified
