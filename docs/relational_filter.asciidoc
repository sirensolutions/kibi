...
Kibi allows you to _filter_ documents on a dashboard by showing only those that
...
The relational filter visualzation is configured based on the <<kibi-settings-relations,relationships>> between indices defined in the settings tab.
...
Both indices are <<kibi-settings-relations,configured>> so that they are joined on the field `companies` of `article` with the the field `id` of `company`. Then, it is possible to use that configuration in order to create a relational filter that would filter companies based on connected articles (or vice-versa).
